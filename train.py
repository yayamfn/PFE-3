...
# Suppress Optuna's default INFO logging for trials unless it's a warning/error
optuna.logging.set_verbosity(optuna.logging.WARNING)

# Create figures directory
os.makedirs('figures', exist_ok=True)
os.makedirs('models', exist_ok=True) # Directory to save model assets
...
def load_data(url):
    ...
    try:
        ...
        y_original = data['status'] # This is already 0 or 1
        le = LabelEncoder()
        # Fit label encoder to learn classes [0, 1]
        # Transform is not strictly necessary if y_original is already 0,1 int
        # But good practice if status could be other types/values
...
def plot_class_distribution(y, le):
    ...
    """
    try:
        # Ensure class names are strings for unambiguous plotting
        class_labels = [str(cls) for cls in le.classes_]
        
        plt.figure(figsize=(8, 6))
        # sns.countplot expects x to be the data array for counting
        ax = sns.countplot(x=y)
        # Set x-tick labels using the stringified class labels
        # Ticks should correspond to the unique values in y (e.g., 0, 1)
...
def preprocess_data(X, y, k_best=10):
    ...
    """
    # Filter specific warning from imblearn/sklearn about feature names
...
def objective_rf(trial, X_train, y_train):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X_train, y_train)):
        ...
        if len(np.unique(y_fold)) < 2:
            # Logged by Optuna if verbosity is high enough, or by main logger
            # logger.warning("Skipping RF fold %d: Only one class present.", fold_idx)
            continue
        try:
...
def objective_xgb(trial, X_train, y_train):
    ...
    model = XGBClassifier(
        ...
        eval_metric='logloss',
        # use_label_encoder=False, # Deprecated and default False in newer XGBoost
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X_train, y_train)):
        ...
        X_val, y_val = X_train[val_idx], y_train[val_idx]
        if len(np.unique(y_fold)) < 2:
            # logger.warning("Skipping XGB fold %d: Only one class present.", fold_idx)
            continue
        try:
            # Removed early_stopping_rounds from here due to user's reported TypeError.
            # This suggests a potential version/environment specific issue with XGBoost/Python 3.13.
            # If early stopping is desired within CV, ensure environment compatibility.
            # model.fit(X_fold, y_fold, eval_set=[(X_val, y_val)], early_stopping_rounds=10, verbose=False)
...
def evaluate_model(model, X_test, y_test, model_name, le):
    try:
        ...
        # Ensure class names are strings for classification report
        ...
        # Convert numpy types in report_dict to native Python types for JSON serialization
        ...
        # raise # Optionally re-raise if you want script to stop
...
def interpret_model(model, X_test_selected, selected_feature_names, model_name="XGBoost"):
    try:
        ...
        # Check if X_test_selected is empty or has too few samples for SHAP
        ...
        if "XGBoost" in model_name or "RandomForest" in model_name or "LGBM" in model_name:
             ...
             # KernelExplainer usually needs a background dataset, X_train_selected would be better
             # For simplicity, we use a subset of X_test_selected if it's large enough
        ...
        if isinstance(shap_values, list): # For classifiers, shap_values is often a list (one per class)
             # For binary classification, typically use shap_values for the positive class (class 1)
             if len(shap_values) == 2:
                 shap_values_for_plot = shap_values[1]
             else: # Multi-class or unexpected format
                 logger.warning("SHAP values list has unexpected length %d. Using first element for summary plot.", len(shap_values))
                 shap_values_for_plot = shap_values[0]
        else:
             shap_values_for_plot = shap_values

        # Check if shap_values_for_plot is suitable for summary_plot
...
def run_internal_tests(model, X_test_original, y_test, scaler, selector, original_feature_names, le):
    ...
    try:
        ...
        for i, idx in enumerate(sample_indices):
            ...
            if hasattr(model, 'predict_proba'):
                # For binary classification, probability of the PREDICTED class
...
if __name__ == '__main__':
    # Suppress specific XGBoost UserWarning about 'use_label_encoder'
    # This is often seen when use_label_encoder=False is passed but not strictly needed by newer versions.
    ...
    # Filter the Matplotlib categorical units warning if labels are numeric-like strings but intended as categories
    ...
    warnings.resetwarnings() # Reset warnings to default at the very end
